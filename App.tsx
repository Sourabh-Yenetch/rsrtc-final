  ...
  const parseQueryWithAI = useCallback(async (text: string) => {
    ...
    try {
      ...
      if (result.to && result.language) {
        ...
        throw new Error("Could not parse destination city and language from response.");
      }
    } catch (e) {
...
const searchBuses = useCallback(() => {
    if (fromCity && toCity) {
        ...
        setTimeout(() => {
            ...
            const foundBuses = DUMMY_BUSES.filter((bus) => {
                const fromCityValue = typeof bus.from === 'object'
            ...
            if (foundBuses.length > 0) {
                ...
                const fromLocationName = typeof firstBus.from === 'object' ? firstBus.from[currentLangKey] : firstBus.from;
                const toLocationName = typeof firstBus.to === 'object' ? firstBus.to[currentLangKey] : firstBus.to;

                const busWord = foundBuses.length === 1
                    ? (lang === Language.ENGLISH ? 'bus' : 'बस')
                    : (lang === Language.ENGLISH ? 'buses' : 'बसें');
    ...
    <div className="min-h-screen bg-gray-50 text-slate-800 font-sans">
      <header className="bg-white shadow-md p-4 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <img src="https://pbs.twimg.com/profile_images/1329774584384503810/U0Ors9EU_400x400.png" alt="RSRTC Logo" className="h-10 w-10 object-contain" />
          <h1 className="text-xl sm:text-2xl font-bold text-indigo-600">{t.title}</h1>
        </div>
      ...
      <main className="container mx-auto p-4 md:p-8">
        <div className="max-w-2xl mx-auto mb-8">
            <div className="w-full h-48 bg-gray-200 rounded-lg flex items-center justify-center text-gray-500 font-semibold text-lg">
                <img src="./assests/rsrtc-banner.jpg" alt="banner" srcset="" />
            </div>
        </div>
